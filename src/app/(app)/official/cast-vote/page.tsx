...
export default function OfficialCastVotePage() {
  ...
  const handleVoterLookup = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success && result.data?.isRecognized && result.data.user) {
        ...
        setError('Voter not found with this National ID.');
      }
    });
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Cast Vote for Citizen</h1>
      ...
      {!voter ? (
        <Card className="mx-auto w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Enter the voter's National ID to verify their identity.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
